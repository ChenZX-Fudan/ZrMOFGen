...
def run_template(template):
	...
	for net in ct2g(template):
		...
		node_cns = [(cncalc(node, 'nodes'), node) for node in os.listdir(os.path.join(BASE_PATH, 'nodes'))]
		...
		if COMBINATORIAL_EDGE_ASSIGNMENT:
			...
			edge_files = sorted([e for e in os.listdir(os.path.join(BASE_PATH, 'edges'))])
			eas = []
			i = 0
...
if __name__ == '__main__':
    ...
    for d in [os.path.join(BASE_PATH, 'templates'), 
              os.path.join(BASE_PATH, 'nodes'), 
              os.path.join(BASE_PATH, 'edges')]:
        try:
