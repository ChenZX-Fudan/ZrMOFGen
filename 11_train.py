...
from train.train_utils import load_state, train, evaluate_model
from train.data_utils import get_local_rank, DataModuleCrystal
from train.loss_utils import get_loss_func_crystal
from train.dist_utils import WandbLogger, init_distributed, increase_l2_fetch_granularity
...
def get_parser():
    ...
    parser.add_argument('--data_dir', type=str, default="/MOFs/hMOF")
    parser.add_argument('--ase_save_dir', type=str, default="/MOFs/hMOF/ase_run")
    parser.add_argument('--data_dir_crystal', type=str, default="/MOFs/hMOF")
    ...
    parser.add_argument('--use_tensors', action="store_true", default=True) #for data, use DGL or PyG formats?
    parser.add_argument('--crystal', action="store_true", default=True) #for data, use DGL or PyG formats?
    ...
    parser.add_argument('--truncate_above', type=lambda x: None if x.lower() == 'none' else float(x), default=None, help="property of Crystal data truncation cutoff...")
...
def run(opt): 
    ...
    try:
        ...
        print(f"Data loaded successfully: train={len(train_loader)}, val={len(val_loader)}, test={len(test_loader)}")
    except Exception as e:
        print(f"Data loaded failed: {e}")
        return
    ...
    if opt.crystal and opt.dataset in ["cifdata"]:
        ...
        eval_results = evaluate_model(opt, trained_model, test_loader, logger)

    else:
        print(f"Conditions not meet:: crystal={opt.crystal}, dataset={opt.dataset}")
        print("Training not executed")
...
# Single GPU Training
# # python main.py --which_mode train --backbone cgcnn --gpu --name my_cgcnn_model --data_dir_crystal /path/to/your/data

# Multi GPU Training
# torchrun --nproc_per_node=2 main.py --which_mode train --backbone cgcnn --gpu --data_dir_crystal /path/to/your/data

# Training with log
# python main.py --which_mode train --backbone cgcnn --gpu --log --data_dir_crystal /path/to/your/data

# Required parameters
# --which_mode train      # mode
# --backbone cgcnn        # CGCNN model
# --data_dir_crystal      # data path
