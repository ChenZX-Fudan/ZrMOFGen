...
def _get_split_sizes(train_frac: float, full_dataset: Dataset) -> Tuple[int, int, int]:
    ...
    len_test = int(0.5 * (len_full - len_train))
...
class CIFData(Dataset):
    ...
    def __init__(self, root_dir=os.path.join(os.getcwd(), "cif_files"), max_num_nbr=12, radius=8, dmin=0, step=0.2,
        ...
        print(f"Truncate_above is {truncate_above is not None}...")
...
def data_augmentation(opt: argparse.ArgumentParser, full_dataset: Dataset):
    ...
    print(f"Originally {len(top_indices)} Top-MOFs. Replicated {np.sort(train_idx_values[train_idx_values > 1]).sum()} times...")
    print(f"Data enriched by {100 * (np.sort(train_idx_values[train_idx_values > 1]).sum()) / (len(all_indices))} percent...")
